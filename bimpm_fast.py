...
import pandas as pd
import string
quora=pd.read_csv('data/new_quora.tsv',sep='\t',header=None,index_col=0)
quora.columns=['question1','question2','is_duplicate']
quora['ID']=quora.index.astype('int')
...
# In[15]:
def bceloss(o,t):
    return -sum((t[:,0]*np.log(o[:,0]+10**(-7)))+(1-t[:,0])*(np.log(1-o[:,0]+10**(-7)))+(t[:,1]*np.log(o[:,1]+10**(-7)))+((1-t[:,1])*np.log(1-o[:,1]+10**(-7))))/t.shape[1]/t.shape[0]
...
def evaluate(model, e_iter):
    ...
    for input_pairs in e_iter:
        ...
        k+=1
    return (1-correct / float(total),loss/ k)
    ...
    while epoch <= num_epochs:
        ...
        if step % total_batches == 0:
            ...
            if (epoch % 3==0):
                ...
                path=str(current_time)[:11]+'/'
                np.savetxt(path+'train_losses.txt',losses)
                np.savetxt(path+'dev_accs.txt',accs)
                np.savetxt(path+'dev_losses.txt',dev_losses)
                np.savetxt(path+'test_accs.txt',test_accs)
                np.savetxt(path+'test_losses.txt',test_losses)
        step += 1
